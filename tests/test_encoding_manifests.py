...
# The one entry here — a manifest left at us-ca/guidance/... by the subtree
# absorption, whose attested content appears in no reachable commit here or in
# the pre-absorption rulespec-us-ca history — was deleted by
...
# not history: nothing retired them, so 121 files carried a second record
# still asserting a hash the file no longer had, and reading only the newest
# is what hid them (axiom-encode#1282). Superseded records are now pruned when
# a rule is re-encoded, and `axiom-encode manifest-audit` requires EVERY
# committed record to match — so the newest-wins resolution below no longer
